...
namespace ExecuteJavaScript
{
    class Program
    {
        ...
        static BlockingCollection<WebClient> ClientQueue = new BlockingCollection<WebClient>(10);
       ...
       /*     ReadNote note = new ReadNote("oi");
        ...
        Page3:
          ...
          //  Console.WriteLine(js.PageSource);
            var urlOfNotas = page.DocumentNode.SelectNodes("//a[starts-with(@onclick,'janela')]");
            Console.WriteLine(urlOfNotas.Count);
            string temp;
            List<Uri> urls_to_download = new List<Uri>();
        ...
        private static void Web_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e, WebClient client)
        {
            ReadNote note = new ReadNote(e.Result);
            note.StartAnalysis();
